...
res (n:ns) mp (Node e (ETransient _ t)) =
...
res (n:ns) mp (Node e (EVariants cs)) = (gcall, Program fs (newGs ++ gs), ns1) where
    ...
    isUsed vname = any (elem vname . vnames . nodeLabel . snd)
...
res' :: NameSupply -> [(Conf, Conf)] -> [Graph Conf] -> ([Conf], Program, NameSupply)
res' ns mp = foldl f ([], Program [] [], ns) where
    f (cs, Program fs gs, ns1) t =
      (cs ++ [g], Program (fs ++ fs1) (gs ++ gs1), ns2) where
        (g, Program fs1 gs1, ns2) = res ns1 mp t

isBase e1 (Node _ (EDecompose _ ts)) = any (isBase e1) ts
isBase e1 (Node _ (EVariants cs)) = any (isBase e1 . snd) cs

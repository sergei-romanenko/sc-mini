...
isValue :: Expr -> Bool
isValue (Ctr _ args) = all isValue args
isValue _ = False
...
gDef p gname cname =
  head [g | g@(GDef _ (Pat c _) _ _) <- gDefs p gname, c == cname]

(//) :: Expr -> Subst -> Expr
(Var x) // sub = fromMaybe (Var x) (lookup x sub)
...
vnames' :: Expr -> [Name]
vnames' (Var v) = [v]
vnames' (Ctr _ args)   = concatMap vnames' args
vnames' (FCall _ args) = concatMap vnames' args
vnames' (GCall _ args) = concatMap vnames' args
